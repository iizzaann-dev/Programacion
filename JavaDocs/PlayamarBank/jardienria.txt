...
#Creamos el rol rol_lectura_escritura, rol_escritura, rol_lectura;

drop role if exists 'rol_lectura_escritura', 'rol_escritura', 'rol_lectura';
create role 'rol_lectura_escritura', 'rol_escritura', 'rol_lectura';
...
-- 8 Obtener el código de empleado y el número de clientes que atiende cada representante de ventas
SELECT e.codigo_empleado,COUNT(c.codigo_cliente) AS numero_clientes FROM empleado e JOIN cliente c ON c.codigo_empleado_rep_ventas=e.codigo_empleado WHERE e.puesto='Representante Ventas' GROUP BY e.codigo_empleado;
