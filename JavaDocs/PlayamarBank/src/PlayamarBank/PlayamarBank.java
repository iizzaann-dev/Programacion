...
public class PlayamarBank {
	...
	public void transferir (double cantidad, PlayamarBank destino) throws IllegalArgumentException, IllegalStateException {
		...
		if (cantidad > MAX_DESCUBIERTO) {
			throw new IllegalStateException("No se puede transferir una cantidad de dinero mayor a la descubierta de la cuenta");
		}
		
		if((cantidad + saldoActual) > MAX_DESCUBIERTO) {
			throw new IllegalStateException("No se puede alcanzar el limite descubierto mediante la suma fe la transferencia y el saldo actual de la cuenta");
		}
